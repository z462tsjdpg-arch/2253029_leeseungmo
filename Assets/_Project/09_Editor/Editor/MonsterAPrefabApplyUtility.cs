...
public static class MonsterAPrefabApplyUtility
{
    // Every scene that can have its own MonsterA instance(s). Apply always re-syncs every one
    ...
    // Monster A motion test scene - ends up in all the others too, no matter which instance you
    // clicked Apply from. Add a scene's path here if MonsterA ever gets placed somewhere else.
    ...
    private const string PrefabPath = "Assets/_Project/02_Prefabs/Monsters/MonsterA.prefab";

    public static void ApplySettingsToPrefab(MonsterAActionTestController instance)
    {
        if (instance == null)
        {
            Debug.LogWarning("Monster A controller is not assigned.");
            return;
        }
        ...
        {
            Debug.LogWarning("Monster A must be a prefab instance before applying changes.");
            return;
        }
        ...
        Debug.Log($"Applied Monster A settings to prefab ({prefabPath}) and synced: {string.Join(", ", syncedScenes)}");
    }

    /// <summary>
    /// Forces every MonsterA instance in every scene in SyncedScenePaths back in line with the
    ...
    private static List<string> RevertOverridesEverywhere()
    {
        ...
        foreach (var scenePath in SyncedScenePaths)
        {
            ...
            var controllers = new List<MonsterAActionTestController>();
            foreach (var root in scene.GetRootGameObjects())
            {
                controllers.AddRange(root.GetComponentsInChildren<MonsterAActionTestController>(true));
            }
            ...
            foreach (var controller in controllers)
            {
                ...
                {
                    Debug.LogWarning($"Skipped {controller.gameObject.name} in {scenePath} - it isn't a prefab instance, so it can't be synced from MonsterA.prefab.");
                    continue;
                }
