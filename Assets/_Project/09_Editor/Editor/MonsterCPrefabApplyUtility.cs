...
public static class MonsterCPrefabApplyUtility
{
    // Every scene that can have its own MonsterC instance(s). Apply always re-syncs every one
    ...
    // Monster C motion test scene - ends up in all the others too, no matter which instance you
    // clicked Apply from. Add a scene's path here if MonsterC ever gets placed somewhere else.
    ...
    private const string PrefabPath = "Assets/_Project/02_Prefabs/Monsters/MonsterC.prefab";

    public static void ApplySettingsToPrefab(MonsterCActionTestController instance)
    {
        if (instance == null)
        {
            Debug.LogWarning("Monster C controller is not assigned.");
            return;
        }
        ...
        {
            Debug.LogWarning("Monster C must be a prefab instance before applying changes.");
            return;
        }
        ...
        {
            enterChildren = false;
            // controlMode is excluded the same way previewMotion already is: it's "which mode is
            // this placed instance running in" (MotionPreview vs Gameplay), not a shared art/
            // balance value, so it must stay local to whichever scene/instance it's on - otherwise
            // Apply from a Gameplay instance would flip the motion test scene's monster over to
            // Gameplay mode too.
            if (property.name == "m_Script" || property.name == "previewMotion" || property.name == "controlMode" || MonsterBalanceFieldNames.PerInstanceOnly.Contains(property.name))
                continue;
        ...
        Debug.Log($"Applied Monster C settings to prefab ({prefabPath}) and synced: {string.Join(", ", syncedScenes)}");
    }

    /// <summary>
    /// Forces every MonsterC instance in every scene in SyncedScenePaths back in line with the
    ...
    private static List<string> RevertOverridesEverywhere()
    {
        ...
        foreach (var scenePath in SyncedScenePaths)
        {
            ...
            var controllers = new List<MonsterCActionTestController>();
            foreach (var root in scene.GetRootGameObjects())
            {
                controllers.AddRange(root.GetComponentsInChildren<MonsterCActionTestController>(true));
            }

            foreach (var controller in controllers)
            {
                // A controller that isn't actually connected to any prefab (e.g. an old scene
                // built before MonsterC became a real prefab) can't be reverted at all -
                // RevertPropertyOverride throws a plain ArgumentException (NOT UnityException) in
                // that case, which would otherwise abort this whole loop. Skip it with a warning
                // instead of crashing; fix that scene (e.g. re-run its "Create ... Scene" command)
                // separately.
                if (!PrefabUtility.IsPartOfPrefabInstance(controller.gameObject))
                {
                    Debug.LogWarning($"Skipped {controller.gameObject.name} in {scenePath} - it isn't a prefab instance, so it can't be synced from MonsterC.prefab.");
                    continue;
                }
                ...
                while (instanceProperty.NextVisible(enterInstanceChildren))
                {
                    ...
                    if (instanceProperty.name == "m_Script" || instanceProperty.name == "previewMotion" || instanceProperty.name == "controlMode" || MonsterBalanceFieldNames.PerInstanceOnly.Contains(instanceProperty.name))
                        continue;
